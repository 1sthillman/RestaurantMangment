...
const elements = {
    ...
    waiterCallSound: document.getElementById('waiterCallSound'),
...
function handlePaymentChange(payload) {
    ...
    if (payload.eventType === 'INSERT') {
        ...
        if (appState.currentUser.role === 'waiter' || appState.currentUser.role === 'kitchen') {
            ...
            playNotificationSound(elements.paymentCompleteSound);
        }
...
function handleWaiterCallChange(payload) {
    ...
    if (payload.eventType === 'INSERT') {
        ...
        if (appState.currentUser.role === 'waiter') {
            ...
            if (elements.waiterCallSound) {
                // Önce durdur (başka bir çağrı sesi çalıyorsa)
                elements.waiterCallSound.pause();
                elements.waiterCallSound.currentTime = 0;
                
                // Sesi çal (geliştirilmiş fonksiyon)
                playNotificationSound(elements.waiterCallSound);
            }
...
function showWaiterCallToast(message, tableNumber, callId) {
    ...
    toastElement.innerHTML = `
        ...
        <button id="waiterComingBtn" data-table="${tableNumber}" data-call-id="${callId}" class="ml-4 bg-white text-red-500 px-3 py-1 rounded-md text-sm font-medium hover:bg-red-50">
            Geliyorum
        </button>
    `;
...
function updateOrderFromRealtime(order) {
    ...
    if (order.status === 'new' && appState.currentUser.role === 'kitchen') {
        ...
        playNotificationSound(elements.orderDeliveredSound);
        ...
        playNotificationSound(elements.orderServedSound);
...
function showOrderDetailScreen(order) {
    ...
    elements.detailTableStatus.textContent = `Durum: ${
        ...
        order.status === 'completed' ? 'Tamamlandı' : 'Bilinmiyor'
    }`;
...
async function submitOrder() {
    ...
    try {
        ...
        try {
            ...
            showToast('Masa durumu güncellenirken bir hata oluştu. Lütfen tekrar deneyin.');
            return;
        }
        ...
        try {
            ...
            if (updateTableError) {
                ...
                // Hata olsa da işleme devam et, kritik bir hata değil
            } else {
                console.log('Masa-sipariş ilişkisi başarıyla güncellendi');
            }
        } catch (tableUpdateError) {
            console.error('Masa-sipariş ilişkisi güncellenirken beklenmeyen hata:', tableUpdateError);
            // Hata olsa da işleme devam et, kritik bir hata değil
        }
...
// Bildirim sesi çal - Android'de daha güvenilir çalışması için geliştirilen fonksiyon
function playNotificationSound(soundElement) {
    if (!soundElement) return;
    
    // Ses çalmadan önce ses dosyasını yükle
    soundElement.load();
    
    // Android'de daha iyi çalışması için promise ile ses çalma
    ...
    if (playPromise !== undefined) {
        playPromise.then(_ => {
            ...
            // Kullanıcı etkileşimi sonrası tekrar deneme
            document.addEventListener('click', function tryPlayAfterInteraction() {
                soundElement.play().catch(e => console.error('Yine başarısız:', e));
...
async function completeOrder(orderId) {
    try {
        ...
        // updateTableStatusFromOrder fonksiyonu çağrılarak masa durumu güncellenir
        // Bu fonksiyon masa durumunu 'hazır' olarak ayarlayacak
        await updateTableStatusFromOrder(order.tableNumber, 'tamamlandi');
...
async function completePayment(orderId, tableId, paymentMethod = 'nakit', paidAmount = 0, changeAmount = 0) {
    try {
        ...
        try {
            ...
            console.error('Sipariş durumu güncellenirken beklenmeyen hata:', orderUpdateErr);
            showToast('Sipariş durumu güncellenemedi');
            return false;
        }
        ...
        try {
            ...
            if (tableUpdateError) {
                ...
                showToast('Masa durumu güncellenemedi, ancak ödeme işlemi devam ediyor');
                // Hata olsa da işleme devam et
            } else {
                console.log('Masa durumu güncellendi: bos');
            }
        } catch (tableError) {
            console.error('Masa durumu güncellenirken beklenmeyen hata:', tableError);
            showToast('Masa durumu güncellenemedi, ancak ödeme işlemi devam ediyor');
            // Hata olsa da işleme devam et
        }
        ...
        try {
            ...
            if (paymentError) {
                ...
                showToast('Ödeme kaydı oluşturulamadı, ancak sipariş tamamlandı');
                // Hata olsa da işleme devam et
            } else {
            ...
            console.error('Ödeme kaydı oluşturulurken beklenmeyen hata:', paymentRecordError);
            showToast('Ödeme kaydı oluşturulamadı, ancak sipariş tamamlandı');
            // Hata olsa da işleme devam et
        }
